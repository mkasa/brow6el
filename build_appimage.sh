...
echo "Building AppImage using podman with Ubuntu..."

# Create temporary build script
cat > /tmp/build_in_container_$$.sh << 'INNER_EOF'
set -e
...
# Run the build inside a Ubuntu container using podman with FUSE enabled
podman run --rm --device /dev/fuse --cap-add SYS_ADMIN --security-opt apparmor=unconfined \
  -v "$(pwd)":/workspace -v /tmp/build_in_container_$$.sh:/build_script.sh \
  -w /workspace ubuntu:22.04 bash /build_script.sh

# Cleanup
rm -f /tmp/build_in_container_$$.sh
