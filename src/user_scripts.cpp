...
void UserScriptsManager::loadConfig() {
    ...
    while (std::getline(file, line)) {
        ...
        if (line.find("auto_inject=") == 0) {
            auto_inject_enabled_ = (line.substr(12) == "true" || line.substr(12) == "1");
            continue;
        }
...
bool UserScriptsManager::matchesPattern(const std::string& url, const std::string& pattern) const {
...
std::vector<std::string> UserScriptsManager::getMatchingScripts(const std::string& url) const {
