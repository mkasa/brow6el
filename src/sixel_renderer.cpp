...
SixelRenderer::SixelRenderer(int width, int height, int cell_width, int cell_height) 
    : width_(width), height_(height), 
      cell_width_(cell_width), cell_height_(cell_height),
      output_(nullptr), dither_(nullptr), 
...
void SixelRenderer::render(const void* buffer, int width, int height, bool hasAlpha,
    ...
    // If no dirty rects provided, treat entire screen as dirty for tiled rendering
    // Only do monolithic on first frame (empty prev_buffer_)
    if (prev_buffer_.empty()) {
        renderMonolithic(src_buffer, width, height);
...
void SixelRenderer::renderMonolithic(const unsigned char* buffer, int width, int height) {
...
bool SixelRenderer::isTileDirty(const unsigned char* current_buffer, int buffer_width,
    ...
    for (int y = 0; y < tile_height; y++) {
        ...
        if (memcmp(current_buffer + offset, prev_buffer_.data() + offset, tile_row_size) != 0) {
            return true;
        }
    }
    return false;
...
void SixelRenderer::renderTile(const unsigned char* buffer, int buffer_width, int buffer_height,
                               int tile_x, int tile_y, int tile_width, int tile_height) {
    ...
    // Extract tile data from buffer - use ALIGNED position so data matches cursor position
    ...
    for (int y = 0; y < tile_height; y++) {
        ...
        if (src_row >= buffer_height) break;
        
        int src_offset = (src_row * buffer_width + aligned_tile_x) * 4;  // Use aligned position
    ...
    status = sixel_dither_initialize(tile_dither, tile_buffer, tile_width, tile_height,
                                     SIXEL_PIXELFORMAT_BGRA8888,
                                     SIXEL_LARGE_AUTO, SIXEL_REP_AUTO,
                                     SIXEL_QUALITY_HIGH);
    ...
    status = sixel_encode(tile_buffer, tile_width, tile_height, 4, tile_dither, output_);
